...
def get_all_data():
    return Response(df.to_json(orient='index'), status=200, mimetype="application/json")

@app.route('/api/weather/<month_year>', methods=['GET'])
def get_data_for_month_year(month_year):
    if month_year in df.index:
        return Response(df.loc[month_year].to_json(orient='index'), status=200, mimetype="application/json")
    else:
